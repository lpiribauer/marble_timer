...
class RacecontrolThread(threading.Thread):
    ...
    #                 print(mean)
    #                 # mean = cv2.cvtColor(np.uint8([[mean]]), cv2.COLOR_BGR2LAB)
    #                 # initialize the minimum distance found thus far
    #
    #                 # text = mean  # colorNames[minDist[1]]
    #                 means.append(mean)
    #             elif means:
    #                 if len(means) > 1:
    #                     r = []
    ...
    #                     # cv2.destroyAllWindows()
    #                     # cap.release()
    #                     return mean_g, mean_b, mean_r
    #                 means = []
    #         cv2.waitKey(1)
    #
    #
    # def register_marble(self, cap):
    #     color = self.detect_marble(cap)
    #     return color
    #
    #
    # def race(self, cap, marbles):
    #     while True:
    #         color = self.detect_marble(cap)
    #         minDist = (np.inf, None)
    #         for i, marble in enumerate(marbles):
    #             d = dist.euclidean(marble.color, color)
    #             if d < minDist[0]:
    #                 minDist = (d, i)
    #         print(minDist)
    #         marble = marbles[minDist[1]]
    #         marble.lap_times.append(time.time())
